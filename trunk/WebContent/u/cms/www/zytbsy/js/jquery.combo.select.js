...
(function (factory) {
	...
	$.extend(Plugin.prototype, {
			...
			this.$options.each(function(i, e){
                ...
                var content ="";
                    if(this.innerHTML.length>11){
                         content = this.innerHTML.substring(0,11)+'...';
                    }else {
                        content = this.innerHTML;
					}

				o+='<li class="'+(this.disabled? self.settings.disabledClass : "option-item") + ' ' +(k == self.selectedIndex? self.settings.selectedClass : '')+ '" data-index="'+(k)+'" data-value="'+this.value+'"'+'style="overflow:hidden;white-space:nowrap;text-overflow:ellipsis;-o-text-overflow:ellipsis;"'+'title="'+this.innerHTML+'"'+'>'+ content + '</li>'
				k++;
			})
		...
		clear : function() {
			   this.$input.val('');
			   this.$el.val('');
			   this._getAll().removeClass(this.settings.selectedClass)
		},
		/**
		 * Destroy API
		 */
