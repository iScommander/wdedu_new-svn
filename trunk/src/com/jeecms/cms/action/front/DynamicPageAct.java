...
import com.jeecms.wdedu.entity.TBaseServices;
...
public class DynamicPageAct {
	...
	public static final String TPL_HUIYUANSHEZHI = "tpl.huiyuanshezhi";
	...
	public String index(HttpServletRequest request,HttpServletResponse response, ModelMap model,Integer pageNo, Integer pageSize,String proname, String cityname, String quxianname) {
		CmsSite site = CmsUtils.getSite(request);
		CmsUser user = CmsUtils.getUser(request);
		FrontUtils.frontData(request, model, site);
		if (user!=null){
			if (StringUtils.isEmpty(user.getAttr().get("province_id")) || StringUtils.isEmpty(user.getAttr().get("major_type_id"))){
				String a = "1231";
				model.addAttribute("a",a);

			}
		}
		Finder f = Finder.create("FROM  TScActiveTeacher bean WHERE 1=1 ORDER BY id ");
		Pagination pagination = commonSvc.findPager(f,cpn(pageNo), 3);
		StringBuilder sb = new StringBuilder();
		StringBuilder procity = new StringBuilder();
		sb.append("SELECT * FROM t_sc_active_detail WHERE 1=1 ");
		if (!org.springframework.util.StringUtils.isEmpty(proname) && !"全部".equals(proname)) {
			sb.append(" AND province='" + proname + "' ");
			procity.append(proname);
		}
		if (!org.springframework.util.StringUtils.isEmpty(cityname) && !"全部".equals(cityname)) {
			sb.append(" AND city='" + cityname + "'");
			procity.append("-" + cityname);
		}
		if (!org.springframework.util.StringUtils.isEmpty(quxianname) && !"全部".equals(quxianname)) {
			sb.append(" AND quxian='" + quxianname + "'");
			procity.append("-" + quxianname);
		}
//		sb.append(" AND '" + simformat.format(new Date()) + "'< active_time ");
        TScActiveDetail ts = new TScActiveDetail();
		...
		List<TBaseServices> tBaseServicesList = commonSvc.loadAll(TBaseServices.class);
		model.addAttribute("activeList", activeList);
		model.addAttribute("pagination",pagination);
//		getIpProvince.getIpAddr(request);
	...
	public String indexForWeblogic(HttpServletRequest request,HttpServletResponse response, ModelMap model,Integer pageNo, Integer pageSize,String proname, String cityname, String quxianname) {
		CmsSite site = CmsUtils.getSite(request);
		CmsUser user = CmsUtils.getUser(request);
		FrontUtils.frontData(request, model, site);

		return index(request, response,model,pageNo,pageSize,proname,cityname,quxianname);
	}
	...
	public String content(Integer id, int pageNo, String[] params,
		...
		if (len != 0) {
			...
			for (CmsGroup group : groups) {
				...
				for (CmsGroup userGroup :
						userGroups) {
					if (group.getId().equals(userGroup.getId())) {
						right = true;
						break;
					}
				}
