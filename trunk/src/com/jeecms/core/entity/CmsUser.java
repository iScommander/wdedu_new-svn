...
public class CmsUser extends BaseCmsUser implements PriorityInterface {
	...
	public Set<String> getGroupPerms(){
		Set<String> perms = new HashSet<String>();
		//增加用户组权限
		if (getGroups() != null && getGroups().size() > 0) {
			for (CmsGroup group :getGroups()) {
				perms.addAll(group.getPerms());
//				if (group.getContriChannels() != null) {
//					Set<String> groupPerms = new HashSet<>();
//					for (Channel channel: group.getContriChannels()){
//						//暂将权限存link字段
//						if (channel.getLink() != null) {
//							groupPerms.add(channel.getLink());
//						}
//					}
...
//				}
		...
		if(getGroup() !=null){
			perms.addAll(getGroup().getPerms());
		}
		return perms;
	}
...
//		setVipValidTime(u.getVipValidTime());
		// Alltodo: 2020/4/9 vip失效时间
		setLoginCount(0);
	}
...
//		if(getVipValidTime()!=null){
//			json.put("vipValidTime",DateUtils.parseDateToTimeStr(getVipValidTime()));
//		}else{
//			json.put("vipValidTime","");
//		}
