...
import com.wdedu.entity.common.JeecgEntityTitle;
...
public abstract class GenericBaseCommonDaoImpl<T, PK extends Serializable> implements IGenericBaseCommonDao {
    /**
     * 初始化Log4j的一个实例
     */
	...
	*/
	...
	public List<T> getListByCriteriaQuery(final CriteriaQuery cq, Boolean ispage) {
		...
		if (ispage)
		{
			...
			int offset = PagerUtil.getOffset(allCounts, cq.getCurPage(), cq.getPageSize());
			criteria.setFirstResult(offset);
			criteria.setMaxResults(cq.getPageSize());		
		}
		return criteria.list();
    ...
    *//**
    ...
    public <T> List<T> findBySql(String sql, Class<T> clazz) {
        ...
        for(Map<String,Object> m:mapList){
            try {
                po = clazz.newInstance();
                MyBeanUtils.copyMap2Bean(po, m, StringUtils.EMPTY);
                rsList.add(po);
            }  catch (Exception e) {
                e.printStackTrace();
            }
